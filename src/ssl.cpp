	 ...
	 double cpos, double cneg)
{
  ...
  // if this is need, then set: double* o = z; and update the arrays accordingly
  ...
  while (iter <= max_iter && search)
    {
      cg_iter = trcg(delta, g, s, r, &reach_boundary, n, 
		     Y, X, Id, sizeI, cgitermax, cpos, cneg, 
		     eps_cg);
...
int trcg(double delta, double *g, double *s, double *r, bool *reach_boundary, 
	 ...
	 const int cgitermax, double cpos, double cneg, 
	 ...
	 int* Id, double cpos, double cneg)
{
  ...
  for (i=0;i<l;i++){
    if (z[i] < 1) {
      ...
      z[sizeI] = y[i] * ((y[i]==1)? cpos : cneg) *(z[i]-1);
      Id[sizeI] = i;
      sizeI++;
    }
  }
  ...
  for(i=0;i<sizeI;i++)
    {
      ...
      xTs = ((y[Id[i]]==1)? cpos : cneg) * (ddot_(&w_size, s, &inc, X + Id[i] * w_size, &inc) + s[w_size - 1]);
